...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if ($student_id > 0 && $amount > 0) {
        ...
        try {
            ...
            $message = "Invoice {$inv_num} created for \${$amount}."; $message_type = 'success';
        } catch (Exception $e) { $message = 'Error creating invoice.'; $message_type = 'danger'; }
    }
}

$invoices = [];
try { $invoices = db()->fetchAll("SELECT i.*, u.full_name FROM invoices i LEFT JOIN users u ON i.student_id = u.id WHERE i.tenant_id = ? ORDER BY i.created_at DESC LIMIT 50", [$tenantId]); } catch (Exception $e) {}
$students = [];
...
<div class="app-layout">
    ...
    <main class="main-content">
        <div class="cyber-header"><div class="page-icon-orb"><i class="fas fa-file-invoice-dollar"></i></div><div><h1>Invoices</h1><p>Create and manage student invoices</p></div></div>
        <div class="cyber-content">
            ...
            <div class="card" style="margin-bottom:20px;"><div class="card-body">
                ...
                <form method="POST"><input type="hidden" name="csrf_token" value="<?= $csrf ?>">
                    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:12px; align-items:end;">
                        <div class="form-group"><label>Student</label><select name="student_id" class="form-control" required><option value="">Select</option><?php foreach ($students as $s): ?><option value="<?= $s['id'] ?>"><?= htmlspecialchars($s['full_name']) ?></option><?php endforeach; ?></select></div>
            ...
            <div class="card"><div class="card-body" style="overflow-x:auto;">
                <table class="table"><thead><tr><th>Invoice #</th><th>Student</th><th>Amount</th><th>Paid</th><th>Balance</th><th>Due</th><th>Status</th></tr></thead><tbody>
                <?php if (empty($invoices)): ?><tr><td colspan="7" style="text-align:center;padding:24px;">No invoices yet.</td></tr>
                <?php else: foreach ($invoices as $inv): ?>
                <tr><td><code><?= htmlspecialchars($inv['invoice_number']) ?></code></td><td><?= htmlspecialchars($inv['full_name'] ?? 'N/A') ?></td><td>$<?= number_format($inv['total_amount'], 2) ?></td><td>$<?= number_format($inv['paid_amount'], 2) ?></td><td>$<?= number_format($inv['balance'], 2) ?></td><td><?= $inv['due_date'] ? date('M j, Y', strtotime($inv['due_date'])) : '-' ?></td><td><span class="badge badge-<?= match($inv['status']) { 'paid' => 'success', 'partial' => 'warning', 'cancelled' => 'secondary', default => 'danger' } ?>"><?= ucfirst($inv['status']) ?></span></td></tr>
