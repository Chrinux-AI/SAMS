...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    $data = [
        'tenant_id' => $tenantId,
        'user_id' => intval($_POST['employee_id'] ?? 0),
...
$payroll = [];
try {
    $payroll = db()->fetchAll("SELECT p.*, u.full_name FROM payroll p LEFT JOIN users u ON p.user_id = u.id WHERE p.tenant_id = ? ORDER BY p.created_at DESC LIMIT 100", [$tenantId]);
} catch (Exception $e) {
}
$staff = [];
try {
...
<?php if ($msg): ?><div class="rounded-xl border border-primary/20 bg-primary/10 text-primary px-4 py-3 mb-5 text-sm font-medium"><?= htmlspecialchars($msg) ?></div><?php endif; ?>
...
<section class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-surface-container-lowest rounded-3xl p-6 shadow-sm space-y-4">
        ...
        <button type="button" class="w-full bg-gradient-to-r from-primary to-primary-container text-white py-3 rounded-xl font-bold shadow-sm">
...
<section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="bg-surface-container-low p-6 rounded-3xl">
        ...
        <p class="text-3xl font-headline font-black text-on-surface mt-1">$<?= number_format((float)$grossPayroll, 2) ?></p>
    ...
    <div class="bg-surface-container-low p-6 rounded-3xl">
        ...
        <p class="text-3xl font-headline font-black text-on-surface mt-1">$<?= number_format((float)$totalDeductions, 2) ?></p>
    ...
    <div class="bg-primary text-white p-6 rounded-3xl shadow-sm">
        ...
        <p class="text-3xl font-headline font-black mt-1">$<?= number_format((float)$totalNet, 2) ?></p>
...
<section class="grid grid-cols-1 xl:grid-cols-12 gap-6 mb-8">
    <div class="xl:col-span-8 bg-surface-container-lowest rounded-3xl border border-surface-container-high shadow-sm overflow-hidden">
        <div class="p-6 border-b border-outline-variant/10 flex justify-between items-center bg-surface-container-low/30">
            ...
            <button type="button" class="bg-surface-container-high text-primary px-4 py-2 rounded-xl text-sm font-bold">Export CSV</button>
        </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($payroll as $p): ?>
                            ...
                            <tr class="hover:bg-surface-container-low/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">$<?= number_format((float)($p['basic_salary'] ?? 0), 2) ?></td>
                                <td class="px-6 py-4 text-right text-secondary">+$<?= number_format((float)($p['allowances'] ?? 0), 2) ?></td>
                                <td class="px-6 py-4 text-right text-error">-$<?= number_format((float)($p['deductions'] ?? 0), 2) ?></td>
                                <td class="px-6 py-4 text-right font-bold text-primary">$<?= number_format((float)($p['net_pay'] ?? 0), 2) ?></td>
                                <td class="px-6 py-4">
...
render_accountant_atlas_shell($page_title, 'payroll', $page_content, $_SESSION['full_name'] ?? 'Accountant');
