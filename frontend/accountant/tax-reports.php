...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
  <div class="bg-surface-container-lowest rounded-2xl p-6 border border-surface-container-high shadow-sm text-center">
    ...
    <p class="text-3xl font-extrabold text-primary"><?php echo accountant_currency($taxableIncome); ?></p>
  </div>
  <div class="bg-surface-container-lowest rounded-2xl p-6 border border-surface-container-high shadow-sm text-center">
    <p class="text-[11px] uppercase tracking-widest text-outline font-bold mb-2">Deductible Expenses</p>
    <p class="text-3xl font-extrabold text-tertiary"><?php echo accountant_currency($deductibleExpenses); ?></p>
  </div>
  <div class="bg-surface-container-lowest rounded-2xl p-6 border border-surface-container-high shadow-sm text-center">
    <p class="text-[11px] uppercase tracking-widest text-outline font-bold mb-2">Net Taxable</p>
    <p class="text-3xl font-extrabold text-secondary"><?php echo accountant_currency($netTaxable); ?></p>
  </div>
  <div class="bg-surface-container-lowest rounded-2xl p-6 border border-surface-container-high shadow-sm text-center">
    <p class="text-[11px] uppercase tracking-widest text-outline font-bold mb-2">Payroll Deductions</p>
    <p class="text-3xl font-extrabold text-error"><?php echo accountant_currency($payrollTax); ?></p>
  </div>
</div>
  ...
  <table class="w-full text-sm">
    <tbody class="divide-y divide-surface-container-low">
      <tr>
        ...
        <td class="px-4 py-3 text-right"><?php echo accountant_currency($taxableIncome); ?></td>
      </tr>
      <tr>
        <td class="px-4 py-3 text-outline">Less: Deductible Expenses</td>
        <td class="px-4 py-3 text-right text-error">(<?php echo accountant_currency($deductibleExpenses); ?>)</td>
      </tr>
      <tr>
        <td class="px-4 py-3 font-bold">Net Taxable Income</td>
        <td class="px-4 py-3 text-right font-bold text-secondary"><?php echo accountant_currency($netTaxable); ?></td>
      </tr>
      <tr>
        <td class="px-4 py-3 text-outline">Payroll Tax Withholdings</td>
        <td class="px-4 py-3 text-right"><?php echo accountant_currency($payrollTax); ?></td>
      </tr>
      <tr>
        <td class="px-4 py-3 text-outline">Estimated Tax (30% of Net Taxable)</td>
        <td class="px-4 py-3 text-right"><?php echo accountant_currency($estimatedTaxAt30); ?></td>
      </tr>
    </tbody>
  </table>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
  ...
  <div class="bg-surface-container-low rounded-xl p-5 border border-outline-variant/10">
    ...
    <p class="text-3xl font-headline font-extrabold text-primary"><?php echo accountant_currency($estimatedTaxAt30 + $payrollTax); ?></p>
