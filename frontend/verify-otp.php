    ...
    <div class="w-full max-w-[440px]">
      ...
      <div class="text-center mb-10">
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-xl bg-surface-container-lowest shadow-[0_20px_40px_rgba(0,7,103,0.06)] mb-6">
          <span class="material-symbols-outlined text-primary text-3xl"><?php echo isset($show_password_form) && $show_password_form ? 'lock_open' : 'domain_verification'; ?></span>
        </div>
      ...
      <div class="bg-surface-container-lowest rounded-xl p-8 shadow-[0_20px_40px_rgba(0,7,103,0.06)] border border-outline-variant/10">
        ...
        <?php if (isset($show_password_form) && $show_password_form): ?>
          ...
          <?php if ($message): ?>
            <div class="flex items-start gap-3 p-4 rounded-xl bg-red-50 text-red-700 text-sm mb-6">
              <span class="material-symbols-outlined text-lg mt-0.5">error</span>
              <span><?php echo htmlspecialchars($message); ?></span>
            </div>
          <?php endif; ?>

          <div class="mb-6 p-4 bg-surface-container-low rounded-xl text-xs text-primary">
          ...
          <form method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <input class="w-full bg-surface-container-low border-none rounded-lg py-3.5 px-4 pr-12 text-sm focus:ring-2 focus:ring-primary/20 transition-all text-on-surface" id="password" name="password" type="password" required minlength="8">
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <input class="w-full bg-surface-container-low border-none rounded-lg py-3.5 px-4 pr-12 text-sm focus:ring-2 focus:ring-primary/20 transition-all text-on-surface" id="confirm_password" name="confirm_password" type="password" required>
            ...
            <button type="submit" class="premium-gradient w-full py-4 px-6 rounded-lg text-white font-headline font-bold text-sm tracking-tight shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 group">
              Complete Activation
        ...
        <?php else: ?>
          ...
          <form method="POST" class="space-y-6">
            ...
            <div class="flex justify-between gap-2 my-6">
              <?php for ($i = 0; $i < 6; $i++): ?>
                <input type="text" name="otp[]" maxlength="1" pattern="[0-9]" required class="w-12 h-14 text-center text-xl font-bold bg-surface-container-low border-none rounded-lg focus:ring-2 focus:ring-surface-tint transition-all font-mono otp-digit text-on-surface">
              <?php endfor; ?>
            </div>
