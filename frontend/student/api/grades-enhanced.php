...
function getGradeOverview()
{
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT c.id, c.class_name as class_name, c.class_code, c.credits,
...
function getGradeDetails()
{
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT sg.*, c.class_name as class_name, gc.name as category_name,
...
function getGradeAnalytics()
{
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT c.class_name as class_name, c.class_code,
...
function getGradeGoals()
{
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT gg.*, c.class_name as class_name,
...
function getGradeRecommendations()
{
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT c.id, c.class_name, AVG(sg.percentage) as avg_grade
            FROM classes c
            JOIN class_enrollments ce ON c.id = ce.class_id
            LEFT JOIN student_grades sg ON c.id = sg.class_id AND sg.student_id = ce.student_id AND sg.is_dropped = 0
            WHERE ce.student_id = ?
