...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        $visibility = $_POST['visibility'] ?? 'authenticated';
        if (!in_array($visibility, ['public', 'authenticated', 'role_specific'])) $visibility = 'authenticated';

        if ($action === 'create') {
            db()->query(
                "INSERT INTO notices (title, content, category, priority, target_roles, is_pinned, expires_at, scheduled_at, visibility, created_by)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [$title, $content, $category, $priority, $target_roles, $is_pinned, $expires_at, $scheduled_at, $visibility, $admin_id]
            );
            ...
            db()->query(
                "UPDATE notices SET title=?, content=?, category=?, priority=?, target_roles=?, is_pinned=?, expires_at=?, scheduled_at=?, visibility=?, updated_at=NOW()
                 WHERE id=?",
                [$title, $content, $category, $priority, $target_roles, $is_pinned, $expires_at, $scheduled_at, $visibility, $notice_id]
            );
    ...
    if ($action === 'archive') {
        $notice_id = (int)$_POST['notice_id'];
        db()->query("UPDATE notices SET status='archived' WHERE id=?", [$notice_id]);
    ...
    if ($action === 'toggle_pin') {
        $notice_id = (int)$_POST['notice_id'];
        db()->query("UPDATE notices SET is_pinned = NOT is_pinned WHERE id=?", [$notice_id]);
            ...
            <form method="POST" id="noticeForm">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="datetime-local" name="scheduled_at" id="noticeScheduled" class="cyber-input">
                ...
                <div class="form-group">
                    ...
                    <select name="visibility" id="noticeVisibility" class="cyber-input">
                        <option value="authenticated">Logged-In Users Only</option>
                        <option value="public">Public (visible on updates page without login)</option>
                        <option value="role_specific">Role-Specific Only</option>
                    </select>
                </div>
    ...
    <script>
        ...
        function editNotice(notice) {
            ...
            if (notice.scheduled_at) {
                document.getElementById('noticeScheduled').value = notice.scheduled_at.replace(' ', 'T');
            }
