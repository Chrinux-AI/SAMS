    ...
    public function createClass($class_data)
    {
        try {
            ...
            $class_record = [
                ...
                'class_teacher_id' => $class_data['teacher_id'] ?? null,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_class'])) {
    $class_data = [
        ...
        'class_teacher_id' => $_POST['teacher_id'] ?? null,
    ...
    $result = AdminEditGuarantee::execute('classes', 0, $class_data, function ($data) use ($students) {
        $id = db()->insert('classes', $data);
        if ($id && !empty($students)) {
    ...
    try {
        AuditLogger::log('create_class', 'classes', "Created class: {$class_data['class_name']} ({$class_data['class_code']})", $_SESSION['user_id'] ?? null);
    } catch (\Throwable $e) {
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bulk_classes'])) {
    ...
    try {
        AuditLogger::log('bulk_create_classes', 'classes', "Bulk created " . count($classes_data) . " classes", $_SESSION['user_id'] ?? null);
    } catch (\Throwable $e) {
    }
...
$teachers = db()->fetchAll("SELECT id, CONCAT(first_name, ' ', last_name) as full_name, email FROM users WHERE role = 'teacher' ORDER BY first_name, last_name");
?>
<!DOCTYPE html>
<html lang="en">
