...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($success) {
            ...
            // Log context switch for auditing
            $uid = (int)($_SESSION['user_id'] ?? 0);
            $tenantName = $tenant['institution_name'] ?? ('Tenant #' . $tenant_id);
            if ($uid > 0 && function_exists('log_activity')) {
                log_activity($uid, 'switch_tenant_context', 'tenants', $tenant_id, 'Switched to tenant context: ' . $tenantName);
            }
            ...
            echo json_encode([
                ...
                'tenant_id' => $tenant_id,
                'redirect_url' => 'dashboard.php?tenant_switched=' . urlencode((string)$tenant_id)
            ]);
        } else {
