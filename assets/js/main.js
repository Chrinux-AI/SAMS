...
function ensureProfessionalTheme() {
    ...
    if (Array.from(document.querySelectorAll('link[rel="stylesheet"]')).some((link) => link.href.includes('/assets/css/cyberpunk-ui.css'))) {
        return;
    }
...
function addManifest() {
    ...
    if (!document.querySelector('link[rel="apple-touch-icon"]')) {
        ...
        appleTouchIcon.href = '/attendance/assets/images/icons/icon-192x192.svg';
...
function getSavedTheme() {
    ...
    return SAMS_THEMES.some(theme => theme.id === saved) ? saved : 'default';
}

function applyTheme(themeId) {
    const finalTheme = SAMS_THEMES.some(theme => theme.id === themeId) ? themeId : 'default';
