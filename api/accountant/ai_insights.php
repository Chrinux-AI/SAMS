...
try {
  ...
  $tenantFilterClause = static function (string $table, string $alias = ''): string {
    if (function_exists('table_has_column') && table_has_column($table, 'tenant_id')) {
      $prefix = $alias !== '' ? $alias . '.' : '';
      return " AND {$prefix}tenant_id = :tenant_id ";
    }
    return '';
  };

  $getTenantCurrencyContext = static function (int $tenantId): array {
    $ctx = [
      'currency_code' => 'NGN',
      'currency_symbol' => '₦',
      'locale' => 'en-NG',
    ...
    if (!table_exists('school_tenants') || !table_has_column('school_tenants', 'settings_json')) {
      return $ctx;
    }

    try {
      $row = db()->fetchOne("SELECT settings_json FROM school_tenants WHERE id = ? LIMIT 1", [$tenantId]);
      $settingsRaw = (string)($row['settings_json'] ?? '');
      if ($settingsRaw === '') {
        return $ctx;
      }
      ...
      $currencyCode = strtoupper(trim((string)($settings['currency_code'] ?? $settings['currency'] ?? '')));
      $currencySymbol = trim((string)($settings['currency_symbol'] ?? ''));
      $locale = trim((string)($settings['locale'] ?? ''));

      if ($currencyCode !== '') {
        $ctx['currency_code'] = $currencyCode;
      }
      if ($currencySymbol !== '') {
        $ctx['currency_symbol'] = $currencySymbol;
      }
      if ($locale !== '') {
        $ctx['locale'] = $locale;
      }
    } catch (Throwable $e) {
      error_log('AI insights tenant currency resolve error: ' . $e->getMessage());
    }

    return $ctx;
  };

  $currency = $getTenantCurrencyContext($tenantId);
  $money = static function (float $value) use ($currency): string {
    return $currency['currency_symbol'] . number_format($value, 2);
  };

  $feeAmountCol = table_has_column('fee_payments', 'amount_paid') ? 'amount_paid' : 'amount';
  $feeDateCol = table_has_column('fee_payments', 'payment_date') ? 'payment_date' : 'created_at';
  $expenseDateCol = table_has_column('expenses', 'expense_date') ? 'expense_date' : 'created_at';
  ...
  $revCurrentRes = db()->fetchOne(
    "SELECT SUM({$feeAmountCol}) as total FROM fee_payments WHERE MONTH({$feeDateCol}) = MONTH(CURRENT_DATE()) AND YEAR({$feeDateCol}) = YEAR(CURRENT_DATE())" . $tenantFilterClause('fee_payments'),
    [':tenant_id' => $tenantId]
  );
  ...
  $revLastRes = db()->fetchOne(
    "SELECT SUM({$feeAmountCol}) as total FROM fee_payments WHERE MONTH({$feeDateCol}) = MONTH(CURRENT_DATE() - INTERVAL 1 MONTH) AND YEAR({$feeDateCol}) = YEAR(CURRENT_DATE() - INTERVAL 1 MONTH)" . $tenantFilterClause('fee_payments'),
    [':tenant_id' => $tenantId]
  );
  ...
  $expCurrentRes = db()->fetchOne(
    "SELECT SUM(amount) as total FROM expenses WHERE MONTH({$expenseDateCol}) = MONTH(CURRENT_DATE()) AND YEAR({$expenseDateCol}) = YEAR(CURRENT_DATE())" . $tenantFilterClause('expenses'),
    [':tenant_id' => $tenantId]
  );
  ...
  $expLastRes = db()->fetchOne(
    "SELECT SUM(amount) as total FROM expenses WHERE MONTH({$expenseDateCol}) = MONTH(CURRENT_DATE() - INTERVAL 1 MONTH) AND YEAR({$expenseDateCol}) = YEAR(CURRENT_DATE() - INTERVAL 1 MONTH)" . $tenantFilterClause('expenses'),
    [':tenant_id' => $tenantId]
  );
  ...
  $recRes = db()->fetchOne(
    "SELECT SUM(amount) as total FROM fee_invoices WHERE (status = 'pending' OR status = 'partial')" . $tenantFilterClause('fee_invoices'),
    [':tenant_id' => $tenantId]
  );
  ...
  $payRes = db()->fetchOne(
    "SELECT SUM(total_amount) as total FROM purchase_orders WHERE status = 'pending'" . $tenantFilterClause('purchase_orders'),
    [':tenant_id' => $tenantId]
  );
...
Always format currency values using {$currency['currency_code']} (Symbol: {$currency['currency_symbol']}). Include the symbol in your response text.

REAL-TIME FINANCIAL DATA:
-------------------------
Revenue (Current Month): " . $money($revenueCurrentMonth) . "
Revenue (Last Month): " . $money($revenueLastMonth) . "
Expenses (Current Month): " . $money($expensesCurrentMonth) . "
Expenses (Last Month): " . $money($expensesLastMonth) . "
Outstanding Fee Invoices (Receivables expected): " . $money($receivables) . "
Pending Purchase Orders (Payables owed): " . $money($payables) . "
-------------------------
