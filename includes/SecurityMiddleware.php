...
class SAMS_SecurityMiddleware {
    ...
    public static function logSecurityEvent($event, $details = []) {
        ...
        try {
            $db = db();
            if ($db) {
                $db->query("INSERT INTO security_logs (event, user_id, ip_address, details, created_at) 
                    VALUES ('" . $event . "', " . ($_SESSION['user_id'] ?? 'NULL') . 
                    ", '" . $_SERVER['REMOTE_ADDR'] . "', '" . json_encode($details) . "', NOW())");
            }
        } catch (Exception $e) {
