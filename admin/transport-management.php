...
$total_drivers = db()->fetchOne("SELECT COUNT(*) as count FROM transport_drivers")['count'] ?? 0;
$total_students = db()->fetchOne("SELECT COUNT(*) as count FROM transport_assignments")['count'] ?? 0;

// Get recent transport data
$recent_routes = db()->fetchAll("SELECT * FROM transport_routes ORDER BY created_at DESC LIMIT 5");
$vehicles = db()->fetchAll("SELECT * FROM transport_vehicles ORDER BY vehicle_name LIMIT 10");
$drivers = db()->fetchAll("SELECT * FROM transport_drivers ORDER BY driver_name LIMIT 10");
